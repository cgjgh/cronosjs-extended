import { scheduleTask, CronosExpression, CronosTask, validate, } from "../src/index";
const RealDate = Date;
...
describe("CronosExpression.nextDate() defaults", () => {
    ...
    test("", () => {
        ...
        expect(CronosExpression.parse("* * * * *").nextDate()).toEqual(new Date("2019-04-21T11:24:00Z"));
    });
    test("", () => {
        mockDate("2019-04-21T11:23:45Z");
...
describe("Scheduling tests", () => {
    ...
    test("CronosTask", () => {
        ...
        const fromDate = "2019-04-21T11:23:45Z";
        mockDate(fromDate);
        const task = new CronosTask(CronosExpression.parse("0 23 18/3 * Apr Tue 2019", {
            timezone: 0,
        }));
        task
    ...
    test("Calling .start() while task running", () => {
        ...
        mockDate(fromDate);
        const task = new CronosTask(CronosExpression.parse("0/5 * * * * *", {
            timezone: 0,
        }));
        task
            .on("started", startedCallback)
